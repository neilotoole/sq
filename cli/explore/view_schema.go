...
func (tr *schemaTree) view(focused bool, width, height int) string {
	...
	titleStyle := tr.theme.Title
	if focused {
		titleStyle = tr.theme.TitleFocus
	}
	b.WriteString(titleStyle.Render("Schema (" + tr.handle + ")"))
	b.WriteString("\n")
	...
	return tr.theme.Pane.Width(width).Height(height).MaxHeight(height).Render(b.String())
}
...
func (tr *schemaTree) update(msg tea.KeyMsg, keys keyMap) (needsFetch bool, tableName string) {
	...
	case key.Matches(msg, keys.Space), key.Matches(msg, keys.Enter):
		// Both space and enter expand/collapse: the inspector pane
		// follows the cursor, so there's no separate "open" action.
