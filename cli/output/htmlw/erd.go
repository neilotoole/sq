...
func (w *metadataWriter) writeProvenance(buf *bytes.Buffer) {
	...
	fmt.Fprintf(buf, `<p class="sq-gen">%s · generated by <code>%s</code></p>`+"\n",
		html.EscapeString(w.pr.GeneratedAt.Format(time.RFC3339)),
		htmlSQCommand(w.pr.GeneratedBy))
}

// htmlSQCommand renders cmd for the provenance <code> span, linking the leading
// "sq" program name to sq.io (the link sits inside the <code>). The remainder
// is HTML-escaped.
func htmlSQCommand(cmd string) string {
	const link = `<a href="https://sq.io">sq</a>`
	if rest, ok := strings.CutPrefix(cmd, "sq "); ok {
		return link + " " + html.EscapeString(rest)
	}
	if cmd == "sq" {
		return link
	}
