...
func (w *metadataWriter) writeProvenance(buf *bytes.Buffer) {
	if w.pr.GeneratedAt.IsZero() || w.pr.GeneratedBy == "" {
		return
	}
	fmt.Fprintf(buf, "*%s · generated by %s*\n\n",
		w.pr.GeneratedAt.Format(time.RFC3339), mdSQCommand(w.pr.GeneratedBy))
}

// mdSQCommand renders cmd for the provenance line as code, with the leading
// "sq" linked to sq.io. Markdown can't nest a link inside a single code span,
// so "sq" is its own linked code chip followed by the rest as code.
func mdSQCommand(cmd string) string {
	const link = "[`sq`](https://sq.io)"
	if rest, ok := strings.CutPrefix(cmd, "sq "); ok {
		return link + " " + mdCode(rest)
	}
	if cmd == "sq" {
		return link
	}
