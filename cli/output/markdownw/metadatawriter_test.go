...
// TestMetadataWriter_DBSemver checks that the "DB semver" row is rendered
// when metadata.Source.DBSemver is set, and omitted when it's empty (an
// underivable semver simply skips the row, per writeKVRow's empty guard).
func TestMetadataWriter_DBSemver(t *testing.T) {
	buf := &bytes.Buffer{}
	w := markdownw.NewMetadataWriter(buf, output.NewPrinting())
	require.NoError(t, w.SourceMetadata(&metadata.Source{
		Handle: "@test", Name: "db", Driver: drivertype.Type("sqlite3"),
		DBVersion: "3.45.1", DBSemver: "v3.45.1",
	}, false))
	require.Contains(t, buf.String(), "| DB semver | `v3.45.1` |")

	buf2 := &bytes.Buffer{}
	w2 := markdownw.NewMetadataWriter(buf2, output.NewPrinting())
	require.NoError(t, w2.SourceMetadata(&metadata.Source{
		Handle: "@test", Name: "db", Driver: drivertype.Type("sqlite3"),
		DBVersion: "3.45.1",
	}, false))
	require.NotContains(t, buf2.String(), "DB semver")
}
