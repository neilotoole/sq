...
func preRun(cmd *cobra.Command, ru *run.Run) error {
	...
	// Stamp the output's Printing with generation provenance, so inspect's
	// Markdown/HTML schema documents can render a "generated by" line.
	if outCfg.outPr != nil && len(ru.Invocation) > 0 {
		outCfg.outPr.GeneratedAt = time.Now().UTC()
		outCfg.outPr.GeneratedBy = "sq " + strings.Join(ru.Invocation, " ")
	}
