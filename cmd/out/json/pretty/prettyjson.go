...
func (f *Formatter) FormatRecords(rows []*sqlh.Record) ([]byte, error) {

	items := make([]interface{}, len(rows))

	for i, row := range rows {
		items[i] = recordToOMap(row)
	}
...
func recordToOMap(r *sqlh.Record) *omap {
...
func (f *Formatter) FormatOrderedMap(m omap) ([]byte, error) {
	//var v interface{}
	//err := json.Unmarshal(data, &v)
	//
	//if err != nil {
...
func (f *Formatter) processOrderedMap(m *omap, depth int) string {
	currentIndent := f.generateIndent(depth - 1)
	nextIndent := f.generateIndent(depth)
	rows := []string{}
	//keys := []string{}
