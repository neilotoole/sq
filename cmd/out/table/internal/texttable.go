...
type Table struct {
	...
	borders    Border
	colSize    int
	colTrans   map[int]out.TextTransformer
	cellTrans  map[string]out.TextTransformer
}

// NewWriter returns a new table that writes to writer.
func NewWriter(writer io.Writer) *Table {
	t := &Table{
		...
		colSize:    -1,
		colTrans:   make(map[int]out.TextTransformer),
		cellTrans:  make(map[string]out.TextTransformer),
	}
	return t
}

// SetColTrans sets the column transformer.
func (t *Table) SetColTrans(col int, transform out.TextTransformer) {
	t.colTrans[col] = transform
}

// SetCellTrans sets the cell transformer.
func (t *Table) SetCellTrans(row int, col int, transform out.TextTransformer) {
	t.cellTrans[fmt.Sprintf("[%v][%v]", row, col)] = transform
}

func (t *Table) getCellTrans(row int, col int) out.TextTransformer {
...
func (t Table) getColTrans(col int) out.TextTransformer {
...
func (t Table) printHeading() {
	...
	for i := 0; i <= end; i++ {
		...
		out.Color.Header.Printf("%s %s ", padFunc(h, Space, v), pad)

		//color.Set(out.Attrs.Header)
