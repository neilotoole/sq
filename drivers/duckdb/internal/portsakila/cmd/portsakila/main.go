...
func main() {
	...
	fixtures := []fixtureEntry{
		...
		{name: "type_test", build: writeTypeTestDDL},
	}
...
// writeTypeTestDDL writes the type_test.ddl SQL file. It is not executed here;
// Phase 4 type-mapping tests will execute it.
func writeTypeTestDDL(_ context.Context) error {
	dst := filepath.Join(dstDir, "type_test.ddl")
	content := `-- type_test.ddl exercises every DuckDB type that the driver maps to a kind.Kind.
