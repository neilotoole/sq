...
func narrowColSel(w *Walker, node Node) error {
	...
	case *Segment:
		...
		//for {
		//
		//	lg.Debugf("in loop, got segment %d", seg.SegIndex())
		//	if seg == nil {
		//		return errorf("did not find a previous segment that was selectable")
		//	}
		//
		//	segChildType, err := seg.ChildType()
		//	if err != nil {
		//		return err
		//	}
		//
		//	if segChildType.Implements(reflect.Type(TypeSelectable)) {
		//		colSel := newColSelector(sel.Parent(), sel.ctx)
		//		return ReplaceNode(sel, colSel)
		//	}
		//
