...
func TestResolver_Set(t *testing.T) {
	...
	got, err := env.NewResolver().Resolve(context.Background(), "SQ_TEST_RESOLVE_ME")
...
func TestResolver_Unset(t *testing.T) {
	...
	_, err := env.NewResolver().Resolve(context.Background(), "SQ_TEST_DEFINITELY_NOT_SET_X9F2K")
...
func TestResolver_EmptyStringIsSet(t *testing.T) {
	...
	got, err := env.NewResolver().Resolve(context.Background(), "SQ_TEST_EMPTY_VAR")
	require.NoError(t, err)
	require.Equal(t, "", got)
}
