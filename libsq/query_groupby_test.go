	...
	testCases := []queryTestCase{
		{
			...
			override:     driverMap{drivertype.TypeMy: "SELECT `customer_id`, sum(`amount`) AS `sum(.amount)` FROM `payment` GROUP BY `customer_id`"},
			wantRecCount: 599,
		},
		{
			name:         "alias-gb",
		...
		},
		{
			...
			override:     driverMap{drivertype.TypeMy: "SELECT `customer_id`, `staff_id`, sum(`amount`) AS `sum(.amount)` FROM `payment` GROUP BY `customer_id`, `staff_id`"},
			wantRecCount: 1198,
		},
		{
		...
		},
		{
			...
			override:     driverMap{drivertype.TypeMy: "SELECT `customer_id`, sum(`amount`) AS `sum(.amount)` FROM `payment` GROUP BY `customer_id` HAVING sum(`amount`) > 100"},
			wantRecCount: 395,
		},
	}
