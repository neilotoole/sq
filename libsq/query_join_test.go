...
func TestQuery_join_inner(t *testing.T) {
	testCases := []queryTestCase{
		...
		},
		{
			...
			override: driverMap{
				drivertype.MySQL:      "SELECT * FROM `store` AS `s` INNER JOIN `address` AS `a` ON `s`.`address_id` = `a`.`address_id`",
				drivertype.ClickHouse: "SELECT * FROM `store` AS `s` INNER JOIN `address` AS `a` ON `s`.`address_id` = `a`.`address_id`",
			},
			wantRecCount:  2,
		...
		},
		{
			...
			override: driverMap{
				drivertype.MySQL:      "SELECT `first_name`, `last_name`, `title` FROM `actor` AS `a` INNER JOIN `film_actor` AS `fa` ON `a`.`actor_id` = `fa`.`actor_id` INNER JOIN `film` AS `f` ON `fa`.`film_id` = `f`.`film_id`",
				drivertype.ClickHouse: "SELECT `first_name`, `last_name`, `title` FROM `actor` AS `a` INNER JOIN `film_actor` AS `fa` ON `a`.`actor_id` = `fa`.`actor_id` INNER JOIN `film` AS `f` ON `fa`.`film_id` = `f`.`film_id`",
			},
		...
		},
		{
			...
			override: driverMap{
				drivertype.MySQL:      "SELECT `a`.`first_name`, `a`.`last_name`, `f`.`title` FROM `actor` AS `a` INNER JOIN `film_actor` AS `fa` ON `a`.`actor_id` = `fa`.`actor_id` INNER JOIN `film` AS `f` ON `fa`.`film_id` = `f`.`film_id`",
				drivertype.ClickHouse: "SELECT `a`.`first_name`, `a`.`last_name`, `f`.`title` FROM `actor` AS `a` INNER JOIN `film_actor` AS `fa` ON `a`.`actor_id` = `fa`.`actor_id` INNER JOIN `film` AS `f` ON `fa`.`film_id` = `f`.`film_id`",
			},
			wantRecCount:  sakila.TblFilmActorCount,
			repeatReplace: innerJoins,
		},
		{
			...
			override: driverMap{
				drivertype.MySQL:      "SELECT * FROM `store` INNER JOIN `address` ON `store`.`address_id` = `address`.`address_id`",
				drivertype.ClickHouse: "SELECT * FROM `store` INNER JOIN `address` ON `store`.`address_id` = `address`.`address_id`",
			},
			wantRecCount:  2,
			repeatReplace: innerJoins,
		},
		...
		},
		{
			...
			override: driverMap{
				drivertype.MySQL:      "SELECT * FROM `store` CROSS JOIN `address`",
				drivertype.ClickHouse: "SELECT * FROM `store` CROSS JOIN `address`",
			},
			wantRecCount:  1206,
	...
	testCases := []queryTestCase{
		{
			...
			override: driverMap{
				drivertype.MySQL:      "SELECT * FROM `actor` LEFT JOIN `film_actor` ON `actor`.`actor_id` = `film_actor`.`actor_id`",
				drivertype.ClickHouse: "SELECT * FROM `actor` LEFT JOIN `film_actor` ON `actor`.`actor_id` = `film_actor`.`actor_id`",
			},
			wantRecCount:  sakila.TblFilmActorCount,
		...
		},
		{
			...
			override: driverMap{
				drivertype.MySQL:      "SELECT * FROM `actor` LEFT OUTER JOIN `film_actor` ON `actor`.`actor_id` = `film_actor`.`actor_id`",
				drivertype.ClickHouse: "SELECT * FROM `actor` LEFT OUTER JOIN `film_actor` ON `actor`.`actor_id` = `film_actor`.`actor_id`",
			},
			wantRecCount:  sakila.TblFilmActorCount,
		...
		},
		{
			...
			override: driverMap{
				drivertype.MySQL:      "SELECT * FROM `actor` RIGHT JOIN `film_actor` ON `actor`.`actor_id` = `film_actor`.`actor_id`",
				drivertype.ClickHouse: "SELECT * FROM `actor` RIGHT JOIN `film_actor` ON `actor`.`actor_id` = `film_actor`.`actor_id`",
			},
			wantRecCount:  sakila.TblFilmActorCount,
		...
		},
		{
			...
			override: driverMap{
				drivertype.MySQL:      "SELECT * FROM `actor` RIGHT OUTER JOIN `film_actor` ON `actor`.`actor_id` = `film_actor`.`actor_id`",
				drivertype.ClickHouse: "SELECT * FROM `actor` RIGHT OUTER JOIN `film_actor` ON `actor`.`actor_id` = `film_actor`.`actor_id`",
			},
			wantRecCount:  sakila.TblFilmActorCount,
		...
		},
		{
			...
			override: driverMap{
				drivertype.MySQL:      "SELECT * FROM `store` CROSS JOIN `staff`",
				drivertype.ClickHouse: "SELECT * FROM `store` CROSS JOIN `staff`",
			},
			wantRecCount:  4,
			repeatReplace: []string{string(jointype.Cross), jointype.CrossAlias},
		},
...
func TestQuery_table_alias(t *testing.T) {
	testCases := []queryTestCase{
		...
		},
		{
			...
			override: driverMap{
				drivertype.MySQL:      "SELECT `oy vey`.`first_name` AS `given_name` FROM `actor` AS `oy vey`",
				drivertype.ClickHouse: "SELECT `oy vey`.`first_name` AS `given_name` FROM `actor` AS `oy vey`",
			},
			wantRecCount: sakila.TblActorCount,
			sinkFns: []SinkTestFunc{
		...
		},
		{
			...
			override: driverMap{
				drivertype.MySQL:      "SELECT `oy vey`.`first_name` AS `oy vey` FROM `actor` AS `oy vey`",
				drivertype.ClickHouse: "SELECT `oy vey`.`first_name` AS `oy vey` FROM `actor` AS `oy vey`",
			},
			wantRecCount: sakila.TblActorCount,
			sinkFns: []SinkTestFunc{
