	...
	testCases := []queryTestCase{
		{
			...
			override:     driverMap{drivertype.TypeMy: "SELECT * FROM `actor` WHERE `actor_id` = 100"},
			wantRecCount: 1,
		},
		{
			name:         "operator/ne",
		...
		},
		{
			...
			override:     driverMap{drivertype.TypeMy: "SELECT * FROM `actor` WHERE `actor_id` >= 100 AND `actor_id` < 150"},
			wantRecCount: 50,
		},
		{
			...
			override:     driverMap{drivertype.TypeMy: "SELECT * FROM `actor` WHERE `actor_id` >= 100 OR (`actor_id` < 150 AND `first_name` = 'TOM')"},
			wantRecCount: 103,
		},
		{
			...
			override:     driverMap{drivertype.TypeMy: "SELECT `first_name` AS `given_name` FROM `actor` WHERE `given_name` = 'TOM'"},
			wantRecCount: 2,
