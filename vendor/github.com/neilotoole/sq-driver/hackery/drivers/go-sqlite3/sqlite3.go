...
func (rc *SQLiteRows) ColumnTypes() []driver.ColumnType {

	cols := rc.Columns()
	colTypes := make([]driver.ColumnType, len(cols))
	dataTypes := make([]driver.FieldType, len(cols))

	declTypes := rc.DeclTypes()

	for i := range colTypes {
		colTypes[i] = driver.ColumnType{
			TableName: "",
			Name:      cols[i],
			Flags:     0,
			FieldType: driver.FieldTypeString, // default
			Decimals:  0,
		}
		...
		case "int", "integer":
			colTypes[i].FieldType = driver.FieldTypeLong
		case "boolean":
			colTypes[i].FieldType = driver.FieldTypeLong
		case "real":
			colTypes[i].FieldType = driver.FieldTypeDouble
		case "blob":
			colTypes[i].FieldType = driver.FieldTypeBLOB
			colTypes[i].Flags = driver.FlagBinary
		case "timestamp", "datetime", "date":
			dataTypes[i] = driver.FieldTypeDateTime
		case "text":
			dataTypes[i] = driver.FieldTypeString
		default:
			dataTypes[i] = driver.FieldTypeString
		}
